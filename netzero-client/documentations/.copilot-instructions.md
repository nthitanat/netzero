...
> **🚨 CRITICAL:** Before writing ANY code, read [documentations/API-STANDARDS.md](./documentations/API-STANDARDS.md) for mandatory patterns.

## 🔒 Mandatory Standards (MUST FOLLOW)

### **1. API Service Pattern (NON-NEGOTIABLE)**
- ✅ All services use instance pattern (NO static methods)
- ✅ Export singleton: `export const service = new Service()`
- ✅ Export default: `export default service`
- ❌ NEVER export class: `export { Service }`
- ❌ NEVER use method destructuring exports

### **2. Import Pattern (NON-NEGOTIABLE)**
- ✅ Always import via barrel: `import { authService } from '../../api'`
- ❌ NEVER direct import: `import { authService } from '../../api/auth'`

### **3. localStorage Pattern (NON-NEGOTIABLE)**
- ✅ Always use storageService: `storageService.getAuthToken()`
- ❌ NEVER direct access: `localStorage.getItem('authToken')`

### **4. Authentication Pattern (NON-NEGOTIABLE)**
- ✅ Components use AuthContext: `const { user } = useAuth()`
- ❌ NEVER import authService in components

**📚 Full Documentation:** 
- [API Standards](./documentations/API-STANDARDS.md) - Mandatory patterns
- [Quick Reference](./documentations/QUICK-REFERENCE.md) - Fast lookup
- [Documentation Hub](./documentations/README.md) - Complete index
...
### 🚨 MANDATORY: Read API-STANDARDS.md First

**Before writing ANY API service code, read [documentations/API-STANDARDS.md](./documentations/API-STANDARDS.md)**

All API services MUST follow these patterns:

### Service Class Pattern (REQUIRED)
```javascript
// api/serviceName.js
...
import { ApiError, API_STATUS } from '../../api';

// ❌ NEVER import directly
import { authService } from '../../api/auth'; // WRONG!
```

### localStorage Pattern (REQUIRED)
```javascript
...
### Authentication in Components (REQUIRED)
```javascript
...
// ❌ NEVER import authService in components
import { authService } from '../../api'; // WRONG in components!
```
...
axiosInstance.interceptors.request.use(
  (config) => {
    const token = storageService.getAuthToken(); // Use storageService!
    if (token) {
