...
const SURVEYMONKEY_BASE_URL = config.surveyMonkey.baseUrl;
...
const EMAIL_SCAN_PER_PAGE = config.surveyMonkey.emailScanPageSize;
const EMAIL_SCAN_MAX_PAGES = config.surveyMonkey.emailScanMaxPages;

function createSurveyMonkeyRateLimitError() {
  const error = new Error('SurveyMonkey rate limit exceeded');
  error.name = 'SurveyMonkeyRateLimitError';
...
async function request(path, options = {}) {
  ...
  if (response.status === 429) {
    throw createSurveyMonkeyRateLimitError();
  }
...
module.exports = {
  findResponseByEmail,
  getResponse,
  createSurveyMonkeyRateLimitError,
  _private: {
    getResponseEmail,
